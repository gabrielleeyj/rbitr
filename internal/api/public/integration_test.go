	...
	for _, tc := range cases {
		...
		WHERE tc.tenant_id = $1`)).
			...
			if tc.expectedDecision == "ALLOW" {
				sm.ExpectQuery(regexp.QuoteMeta(`SELECT tool_id, tenant_id, base_url, auth_type, auth_value, transport, mcp_upstream_url, description, input_schema_json, archived_at, source FROM rbitr.tools WHERE tenant_id = $1 AND tool_id = $2`)).
					WithArgs("t_demo", "mock_internal").
					WillReturnRows(sqlmock.NewRows([]string{"tool_id", "tenant_id", "base_url", "auth_type", "auth_value", "transport", "mcp_upstream_url", "description", "input_schema_json", "archived_at", "source"}).
						AddRow("mock_internal", "t_demo", "http://mock.local", "", "", "http", nil, nil, nil, nil, "admin"))
			}
